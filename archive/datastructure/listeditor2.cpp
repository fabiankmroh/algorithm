...
void remove_node(ListNode **phead, ListNode *p, ListNode *removed, ListNode **prear){
    if(p == NULL){ /* Removed is the first node */
        *phead = (*phead)->rlink;
        (**prear->rlink) = **phead;
    }
    else{
        p->rlink = removed->rlink;
        (removed->rlink)->llink = p;
    }
...
void Delete(LinkedListType *list, int pos){
    if(!is_empty(list) && (pos >= 0) && (pos < list->length)){
        ...
        if(list->length == 0){
            ...
            end = get_node_at(list, list->length);
        }
        remove_node(&(list->head), p, (p != NULL)?p->rlink: NULL, &end);
        list->length--;
    }
}
